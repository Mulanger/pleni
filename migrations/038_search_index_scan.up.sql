-- Let pgvector iterate through filters; avoid materializing whole transcripts on disk.
-- Publication predicates match feed_clip_catalogue; document FKs retain source integrity.
...
begin
  ...
  -- Loading the vector type above registers its transaction-local tuning parameters.
  ...
  with eligible as not materialized (
    ...
    where (p_person is null or d.politician_id=p_person)
      ...
      and c.published_at<=p_snapshot and c.moderation<>'rejected' and c.url_540x960<>''
  ), lexical as not materialized (
    select d.*,
    ...
    where vector is not null and topic is not null and topic !~ '["“”]'
      and ch.index_version=search_v2.index_version
      and (select true from eligible d where d.clip_id=ch.clip_id
        and d.semantic_state='current' and d.completed_index_version=search_v2.index_version
        and ch.source_hash=d.source_hash limit 1)
    order by ch.embedding operator(extensions.<=>) vector
    limit 240
