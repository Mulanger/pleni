-- 029_search_candidate_admission
--
-- OPT2 ranking v3. Additive: it adds candidate-level semantic admission next to
-- the existing v2 RPC, which stays deployed and callable so an Edge Function
-- rollback needs no SQL. Migrations 022 through 028 are left untouched.
--
-- v2 admits every semantic candidate as soon as the query as a whole clears the
-- safety gate, so one strong candidate can drag weak context-only filler into a
-- valid result list. v3 keeps that query-level gate and then requires each
-- semantic-only candidate to clear its own similarity or Swedish lexical
-- coverage bar. Keyword-matched candidates are never filtered.
--
-- The two candidate constants below were selected offline by
-- `scripts/evaluate_topic_search.py admission-grid` against the frozen capture,
-- using the roadmap's conservative order. They are engineering evidence of
-- observable membership, not a human-judged relevance result. They are mirrored
-- in supabase/functions/_shared/search/ranking.ts and a test fails on drift.
--
-- A result list may be shorter than the requested limit. Nothing backfills it.

do $preflight$
begin
  if to_regprocedure(
    'public.search_clip_candidates_v2(text,text,integer,uuid,text,date,date,uuid[])'
  ) is null
    or to_regprocedure('public.prepare_clip_search_request(text)') is null then
    raise exception 'migration 027 search RPCs are missing';
  end if;
end;
$preflight$;

create or replace function public.search_clip_candidates_v3(
...
begin
  ...
  if normalized_topic is null then
    ...
    -- Query-level safety gate, unchanged from v2: does this query have any
    -- semantic footing at all?
    semantic_query_admitted as (
      select candidate.*
      from semantic_ranked candidate
    ...
    -- v3 candidate-level admission. Each semantic-only candidate must clear its
    -- own bar; a strong candidate elsewhere in the query never admits a weaker
    -- one. Keyword-matched candidates are exempt and always survive. Retrieval
    -- ranks were assigned before this filter, so removing a row never reorders
    -- or promotes the rows that remain.
    semantic_admitted as (
      select candidate.*
      from semantic_query_admitted candidate
      where exists (
          select 1
          from keyword_ranked keyword
          where keyword.clip_id = candidate.clip_id
        )
        or candidate.similarity >= 0.50
        or candidate.lexical_coverage >= 0.67
    ),
    fused as (
      select
...
comment on function public.search_clip_candidates_v3(
  text, text, integer, uuid, text, date, date, uuid[]
) is 'OPT2 hybrid retrieval with query-level and candidate-level semantic admission; no score is returned';

revoke all on function public.search_clip_candidates_v3(
  text, text, integer, uuid, text, date, date, uuid[]
) from public, anon, authenticated;
grant execute on function public.search_clip_candidates_v3(
  text, text, integer, uuid, text, date, date, uuid[]
) to service_role;
